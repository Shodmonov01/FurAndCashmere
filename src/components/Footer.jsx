...
function Footer({ categories }) {
    ...
    // Функция перехода по прямой ссылке
    const goToPage = (path) => {
        navigate(path)
    }
    ...
    const navigateToCategory = (item) => {
        navigate(`/catalog/${item.slug}`, { state: item })
    }

    // Функция переключения состояния категории (открыто/закрыто)
    ...
    return (
        <footer className='bg-dark-red pt-[30px] lg:pt-[40px] pb-[20px] lg:pb-[22px] text-[14px] font-inter font-normal'>
            <Layout>
                <div className='grid grid-cols-2 lg:grid-cols-4 text-white gap-[20px] lg:gap-[10px]'>
                    {/* Первая колонка с категориями */}
                    <div className='col-span-1'>
                        <ul className='flex flex-col gap-[20px] lg:gap-[10px]'>
                                ...
                                <li key={item.slug}>
                                    {item.sub_category?.length > 0 ? (
                                        <div className='flex flex-col gap-[20px] lg:gap-[10px]'>
                                            ...
                                            <button 
                                                className='flex items-center gap-4 text-left hover:text-white/80 transition-all w-full'
                                                onClick={() => toggleCategory(item.slug)}
                                            >
                                                <span>{item.name}</span>
                                                <IoIosArrowDown 
                                                    className={`transition-all ${openCategoryIds[item.slug] ? 'rotate-180' : ''}`} 
                                                />
                                            </button>
                                            
                                            {/* Выпадающие подкатегории */}
                                            {openCategoryIds[item.slug] && (
                                                <div className='flex flex-col gap-[10px]'>
                                                    {item.sub_category.map(subItem => (
                                                        <button
                                                            key={subItem.slug}
                                                            className='text-left hover:text-white/80 transition-all w-full'
                                                            onClick={() => navigateToCategory(subItem)}
                                                        >
                                                            {subItem.name}
                                                        </button>
                                                    ))}
                                        ...
                                        <button
                                            className='hover:text-white/80 transition-all text-left w-full'
                    ...
                    {/* Вторая колонка - десктоп версия */}
                    <div className='col-span-1 hidden lg:block'>
                        <ul className='flex flex-col gap-[20px] lg:gap-[10px] text-right lg:text-left'>
                            <li>
                                <button 
                                    className='hover:text-white/80 transition-all text-left w-full'
                                    onClick={() => goToPage('/custom_tailoring')}
                                >
                                    Индивидуальный пошив
                                </button>
                            </li>
                            ...
                            <li>
                                <button 
                                    className='hover:text-white/80 transition-all text-left w-full'
                                    onClick={() => goToPage('/fabric_excellence')}
                                >
                                    Превосходство тканей
                                </button>
                            </li>
                            <li>
                                <button 
                                    className='hover:text-white/80 transition-all text-left w-full'
                                    onClick={() => goToPage('/exclusivity_of_furs')}
                                >
                                    Эксклюзивность мехов
                                </button>
                            </li>
                            ...
                            <li>
                                <div className='flex flex-col gap-[20px] lg:gap-[10px]'>
                                    ...
                                    {openBlog && (
                                        <div className='flex flex-col gap-[10px]'>
                                            <button 
                                                className='hover:text-white/80 transition-all text-right'
                    ...
                    <div className='col-span-1 hidden lg:block'>
                        <ul className='flex flex-col gap-[20px] lg:gap-[10px]'>
                            <li>
                                <div className='flex flex-col gap-[20px] lg:gap-[10px]'>
                                    <button
                                        className='flex items-center gap-4 hover:text-white/80 transition-all text-left w-full'
                                        onClick={() => setOpenBlog(!openBlog)}
                                    >
                                        <span>Блог</span>
                                        <IoIosArrowDown className={`${openBlog ? 'rotate-180' : ''} transition-all`} />
                    ...
                    <div className={`col-span-1 ${openBlog ? '-mt-[30px]' : '-mt-[120px]'} lg:mt-auto`}>
                        <ul className='flex flex-col gap-[20px] lg:gap-[10px] text-right'>
                            <li className='hover:text-white/80 transition-all hidden lg:block'>
                                Санкт Петербург, Петроградский район, м. Горьковская
                            </li>
                            <li className='hover:text-white/80 transition-all block lg:hidden'>
                                Санкт-Петербург, Петроградский район, <br /> м. Горьковская
                            </li>
                            <li className='hover:text-white/80 transition-all'>
                                По предварительной записи
                            </li>
                            <li>
                                <a 
                                    href='mailto:fur.cashmere@mail.ru' 
                                    className='inline-block hover:text-white/80 transition-all'
                                >
                                    fur.cashmere@mail.ru
                                </a>
                            </li>
                        </ul>
                    </div>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-3 items-center lg:items-end pt-[40px] text-white/50'>
                    <button
                        className='col-span-1 text-center lg:text-left hover:text-white/70 transition-all'
                    ...
                    <div className='flex justify-center order-first lg:order-none pb-[30px] lg:pb-0'>
                        <img src={footer_logo} alt='логотип' className='col-span-1 hidden lg:block' />
                        <img src={footer_logo_mobile} alt='логотип' className='col-span-1 block lg:hidden' />
                    </div>
                    
                    <button
                        className='col-span-1 text-center lg:text-right pt-[20px] lg:pt-0 hover:text-white/70 transition-all'
